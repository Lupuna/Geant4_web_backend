...
class ExampleCommandViewSetTestCase(AuthSettingsTest):
    def setUp(self):
        ...
        self.url = reverse('example-example-command-list', kwargs={'example_pk': self.example.id})
    ...
    def test_add_user_in_example_command(self):
        ExampleCommandViewSet._add_user_in_example_command(self.example, self.key_s3, self.user)

        ex_command = ExampleCommand.objects.get(key_s3=self.key_s3, example=self.example)
        self.assertIn(self.user, ex_command.users.all())

    def test_add_user_in_example_command_when_user_already_added(self):
        ex_command = ExampleCommand.objects.create(key_s3=self.key_s3, example=self.example)
    ...
    def test_example_executed(self):
        ...
        request = self.factory.post(self.url, self.params, format='json', example_pk=self.example.id)
        force_authenticate(request, user=self.user)

        self.view.setup(request=request, example_pk=self.example.id)
        ex_commands = self.view.get_queryset().prefetch_related('example', 'users').filter(key_s3='some-key')
        response = ExampleCommandViewSet()._example_executed(ex_commands, self.user)

        self.assertEqual(response.status_code, status.HTTP_200_OK)

        ex_command.refresh_from_db()
        self.assertIn(self.user, ex_command.users.all())
        self.assertEqual(response.data, {'detail': 'Example already executed, wait for results'})
    ...
    def test_run_example_failure(self, mock_post):
        ...
        self.view.setup(request=request, kwargs={'example_pk': self.example.id})

        response = self.view._run_example(request, self.example, self.params['params'])
